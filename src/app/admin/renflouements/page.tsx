      ...
      {loading ? (
        ...
        <div className={styles.noRefueling}>
          ...
          {!selectedExercise.active ? (
            ...
            >
              {calculating ? (
                <><i className="fas fa-spinner fa-spin"></i> Calcul en cours...</>
              ) : (
                <><i className="fas fa-play"></i> Lancer le Calcul du Renflouement</>
              )}
            </button>
          ) : (
            <div style={{ marginTop: '2rem', padding: '1rem', background: '#fffbeb', color: '#92400e', borderRadius: '12px', border: '1px solid #fef3c7', display: 'inline-block' }}>
               <i className="fas fa-exclamation-triangle"></i> L'exercice doit être clôturé avant de calculer le renflouement.
            </div>
          )}
        </div>
      ) : (
