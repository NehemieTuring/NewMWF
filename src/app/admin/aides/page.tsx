...
export default function AidesPage() {
  ...
  const handleCreateHelpType = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(err.message || "Erreur lors de la création du type d'aide.", "error");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <header style={{ marginBottom: "2.5rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: "1rem" }}>
          ...
          {activeTab === "nomenclature" && (
             <button type="button" className={styles.confirmBtn} onClick={() => setShowTypeModal(true)} style={{ background: "linear-gradient(135deg, #4e73df, #224abe)" }}>
                       ...
                       {aides.length === 0 ? (
                         ...
                         <tr key={a.id}>
                            ...
                            <td style={{ textAlign: "right" }}>
                               {a.status !== "DISBURSED" && (
                                 <button type="button" className={styles.confirmBtn} style={{ padding: "0.5rem 1.25rem", fontSize: "0.75rem", fontWeight: 800, background: "linear-gradient(135deg, #4e73df, #224abe)", borderRadius: "10px", boxShadow: "0 4px 10px rgba(78, 115, 223, 0.2)" }} onClick={() => handleDisburse(a.id)}>
                                    DÉCAISSER
                                 </button>
                               )}
                            </td>
          ...
          <div className={`${styles.modal} fade-in-up`} style={{ maxWidth: "550px", borderRadius: "30px", border: "none", overflow: "hidden", boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.25)" }}>
            ...
            <form onSubmit={handleCreateAide} className={styles.modalBody} style={{ padding: "2.5rem" }}>
               ...
               <div className={styles.formGroup} style={{ marginBottom: "1.5rem" }}>
                  ...
                  <input type="number" className={styles.formInput} value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} required style={{ borderRadius: "15px", padding: "1rem 1.25rem", border: "1px solid #e2e8f0", background: "#f8fafc", fontWeight: 700, color: "#2d3748" }} />
               </div>
      ...
      {showTypeModal && (
        <div className={styles.modalOverlay}>
          <div className={`${styles.modal} fade-in-up`} style={{ maxWidth: "550px", borderRadius: "30px", border: "none", overflow: "hidden", boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.25)" }}>
            <div className={styles.modalHeader} style={{ background: "linear-gradient(135deg, #4e73df, #224abe)", padding: "1.75rem 2rem" }}>
              <h3 style={{ color: "white", fontSize: "1.35rem", fontWeight: 800, margin: 0 }}>
                <i className="fas fa-plus-circle" style={{ marginRight: "0.75rem", opacity: 0.8 }}></i>
                Nouveau Type d'Aide
              </h3>
            ...
            <form onSubmit={handleCreateHelpType} className={styles.modalBody} style={{ padding: "2.5rem" }}>
               ...
               <div className={styles.modalActions} style={{ marginTop: "2rem", gap: "1.25rem" }}>
                  ...
                  <button type="submit" className={styles.confirmBtn} disabled={submitting} style={{ background: "linear-gradient(135deg, #4e73df, #224abe)", borderRadius: "16px", padding: "1.1rem", flex: 2, boxShadow: "0 10px 25px rgba(78, 115, 223, 0.3)", border: "none" }}>
                    {submitting ? <i className="fas fa-spinner fa-spin"></i> : <><i className="fas fa-check-circle" style={{ marginRight: "0.6rem" }}></i> Enregistrer le Type</>}
                  </button>
               </div>
            </form>
          </div>
