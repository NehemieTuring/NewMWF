...
export default function AgapePage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [agapeData, sessionData] = await Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast("Agape enregistrée et financée par le Fonds Social.", "success");
      setShowModal(false);
      setForm({
        title: "",
        description: "",
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.glassCard} style={{ background: "rgba(255,255,255,0.7)", padding: "1.5rem", marginBottom: "2rem", border: "1px solid rgba(78, 115, 223, 0.2)" }}>
         <i className="fas fa-info-circle" style={{ color: "#4e73df", marginRight: "0.5rem" }}></i>
         <span style={{ fontSize: "0.9rem", color: "#4a5568" }}>Les agapes sont financées à <b>100%</b> par le fonds social. Aucune cotisation membre n'est requise.</span>
      </div>
        ...
        <div className={styles.modalOverlay}>
          <div className={`${styles.modal} fade-in-up`} style={{ maxWidth: "550px", borderRadius: "30px", border: "none", overflow: "hidden" }}>
            <div className={styles.modalHeader} style={{ background: "linear-gradient(135deg, #1cc88a, #13855c)", padding: "1.75rem 2rem" }}>
              <h3 style={{ color: "white", fontSize: "1.3rem", fontWeight: 800 }}>Organiser une Agape</h3>
              <button className={styles.modalClose} style={{ color: "white", background: "rgba(255,255,255,0.2)" }} onClick={() => setShowModal(false)}>
                <i className="fas fa-times"></i>
              </button>
            </div>
            <form onSubmit={handleSubmit} className={styles.modalBody} style={{ padding: "2rem" }}>
               <div className={styles.formGroup}>
                  <label style={{ color: "#4e73df", fontWeight: 700 }}><i className="fas fa-bullhorn" style={{ marginRight: "0.5rem" }}></i> Titre de l'événement</label>
                  <input type="text" className={styles.formInput} value={form.title} onChange={e => setForm({...form, title: e.target.value})} placeholder="Ex: Grand repas de fin d'exercice" required style={{ borderRadius: "15px", padding: "0.9rem 1.25rem" }} />
               </div>
               <div className={styles.formGroup}>
                  <label style={{ color: "#4e73df", fontWeight: 700 }}><i className="fas fa-align-left" style={{ marginRight: "0.5rem" }}></i> Description</label>
                  <textarea className={styles.formInput} style={{ minHeight: "100px", borderRadius: "15px", padding: "0.9rem 1.25rem" }} value={form.description} onChange={e => setForm({...form, description: e.target.value})} placeholder="Détails du menu ou motif..." required />
               </div>
               <div className={styles.formGroup}>
                  <label style={{ color: "#4e73df", fontWeight: 700 }}><i className="fas fa-money-bill-wave" style={{ marginRight: "0.5rem" }}></i> Budget total (XAF)</label>
                  <input type="number" className={styles.formInput} value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} placeholder="Montant à prélever du Fonds Social" required style={{ borderRadius: "15px", padding: "0.9rem 1.25rem" }} />
               </div>
               <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
                 <div className={styles.formGroup}>
                    <label style={{ color: "#4e73df", fontWeight: 700 }}><i className="fas fa-calendar-day" style={{ marginRight: "0.5rem" }}></i> Date prévue</label>
                    <input type="date" className={styles.formInput} value={form.date} onChange={e => setForm({...form, date: e.target.value})} required style={{ borderRadius: "15px", padding: "0.9rem 1.25rem" }} />
                 </div>
                 <div className={styles.formGroup}>
                    <label style={{ color: "#4e73df", fontWeight: 700 }}><i className="fas fa-clock" style={{ marginRight: "0.5rem" }}></i> Session liée</label>
                    <select className={styles.formInput} value={form.sessionId} onChange={e => setForm({...form, sessionId: e.target.value})} required style={{ borderRadius: "15px", padding: "0.9rem 1.25rem" }}>
