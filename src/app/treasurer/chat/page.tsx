...
export default function TreasurerChat() {
  ...
  useEffect(() => {
    async function loadConversations() {
      try {
        const data = await treasurerService.getConversations();
  ...
  useEffect(() => {
    if (activeConv) {
      async function loadMessages() {
        try {
          const data = await treasurerService.getMessages(activeConv.id);
          setMessages(data || []);
        } catch (err) {
      ...
      loadMessages();
      const interval = setInterval(loadMessages, 5000); // Polling for treasurer chat too
  ...
  const handleSend = async () => {
    ...
    try {
      await treasurerService.sendMessage(activeConv.id, messageText);
      setMessageText("");
      const data = await treasurerService.getMessages(activeConv.id);
      setMessages(data || []);
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className={styles.loading}>
        ...
        <span>Messagerie administrative...</span>
      </div>
    );
  }
